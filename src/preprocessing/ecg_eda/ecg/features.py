...
warnings.filterwarnings("ignore", category=UserWarning, message="Signal duration is to short for segmentation into 300000s.")
warnings.filterwarnings("ignore", category=UserWarning, message="Signal duration too short for SDANN computation.")
warnings.filterwarnings("ignore", category=UserWarning, message="CAUTION: The TINN computation is currently providing incorrect results in the most cases due to a malfunction of the function.")
...
def ecg_features(dataframe: pd.DataFrame,
    ...
    t, filtered_signal, rpeaks = biosppy.signals.ecg.ecg(raw_ecg_signal,
                                                         ...
                                                         show=plot_analysis)[:3]
    signal_features = hrv(rpeaks=t[rpeaks], plot_ecg=plot_ecg, plot_tachogram=plot_tachogram)
    features = {
